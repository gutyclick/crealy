...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json().catch(() => null) as { plan?: unknown; period?: unknown; clientRequestId?: unknown } | null;
    ...
    const session = await getStripeClient().checkout.sessions.create({ mode: "subscription", customer: customerId, client_reference_id: user.id, line_items: [{ price: getStripePriceId(publicPlan, period), quantity: 1 }], success_url: `${siteUrl}/billing/success?session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${siteUrl}/pricing?checkout=cancelled`, allow_promotion_codes: false, metadata: { supabase_user_id: user.id, plan_key: planKey, public_plan: publicPlan, billing_period: period }, subscription_data: { metadata: { supabase_user_id: user.id, plan_key: planKey, public_plan: publicPlan, billing_period: period } } }, { idempotencyKey: `checkout:${user.id}:${body.clientRequestId}` });
