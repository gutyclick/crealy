...
import { Check, ImagePlus, Link2, LoaderCircle, Sparkles, Upload } from "lucide-react";
...
export function RecreatePanel({ category, references, setReferences, disabled, onChange }: {
  ...
  async function prepare(file: File) {
    ...
    try {
      ...
      const payload = await readApiResponse<{ blueprint: RecreateBlueprint } | { error: string }>(response, "No pudimos analizar esta referencia.");
      if (!response.ok || "error" in payload) throw new Error("error" in payload ? payload.error : "No pudimos analizar esta referencia.");
      setStatus("ready"); onChange({ similarity, blueprint: payload.blueprint, ready: true });
    } catch (error) {
  ...
  return <section className="mt-7 overflow-hidden rounded-2xl border border-brand/20 bg-brand/[0.035]">
    ...
    <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[1fr_15rem]">
      <div><label htmlFor="recreate-url" className="text-sm font-semibold text-foreground">Pega un enlace o sube una imagen</label><div className="mt-3 flex gap-2"><div className="relative min-w-0 flex-1"><Link2 className="absolute left-3 top-3.5 size-4 text-muted" /><input id="recreate-url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." className="h-12 w-full rounded-xl bg-background pl-10 pr-3 text-sm text-foreground outline-none ring-1 ring-white/10 focus:ring-brand/65" /></div><button type="button" onClick={loadUrl} disabled={disabled || status === "loading" || status === "analyzing"} className="rounded-xl bg-white px-4 text-sm font-bold text-black disabled:opacity-50">Analizar</button></div>
        <div className="my-4 flex items-center gap-3 text-[11px] uppercase tracking-[.16em] text-muted"><span className="h-px flex-1 bg-white/8" />o<span className="h-px flex-1 bg-white/8" /></div>
        <input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(e) => e.target.files?.[0] && prepare(e.target.files[0])} /><button type="button" onClick={() => inputRef.current?.click()} disabled={disabled} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-white/15 bg-background text-sm font-semibold text-foreground hover:border-brand/45"><Upload className="size-4 text-brand" />Subir imagen</button>
      </div>
      <div className="relative aspect-video overflow-hidden rounded-xl bg-background ring-1 ring-white/10">{source ? <Image src={source.previewUrl} alt="Vista previa de la referencia" fill unoptimized className="object-cover" /> : <div className="grid h-full place-items-center text-center text-xs text-muted"><span><ImagePlus className="mx-auto mb-2 size-6" />Tu referencia aparecerá aquí</span></div>}{status === "loading" || status === "analyzing" ? <div className="absolute inset-0 grid place-items-center bg-black/70 text-xs font-semibold text-white"><span className="flex items-center gap-2"><LoaderCircle className="size-4 animate-spin text-brand" />{status === "loading" ? "Obteniendo imagen" : "Entendiendo la fórmula"}</span></div> : null}</div>
    </div>
    {status === "ready" ? <div className="mx-5 mb-5 flex items-center gap-2 rounded-xl bg-brand/10 px-4 py-3 text-sm font-semibold text-foreground ring-1 ring-brand/25 sm:mx-6 sm:mb-6"><Check className="size-4 text-brand" />Crealy entendió la composición, la jerarquía y la energía visual.</div> : null}
