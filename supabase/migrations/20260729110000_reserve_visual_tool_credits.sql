alter table public.credit_reservations
  drop constraint if exists credit_reservations_reference_type_check;
alter table public.credit_reservations
  add constraint credit_reservations_reference_type_check
  check (reference_type in ('generation', 'edit', 'thumbnail_analysis'));

drop function if exists public.consume_tool_analysis_credit_internal(
  uuid, uuid, integer
);
...
begin
  ...
  if p_reference_type not in (
    'generation', 'edit', 'thumbnail_analysis'
  ) then
